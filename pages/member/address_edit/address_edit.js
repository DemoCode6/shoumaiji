...
Page({
  ...
  formSubmit(e) {
    ...
    data1.is_moren = this.data.is_moren;
    ...
    wx.request({
      url: this.data.globalData.domain + '/wxxcx/member/address_edit',
      header: {
        'content-type': 'application/json' // 默认值
      },
      data: {
        member_id: app.globalData.user_msg.id,
        id:this.data.view.res.id,
        data: data1
      },
      method: "post",
          ...
          wx.showToast({
            ...
            success: function (res) {
              if (that.data.shop_id) {
                wx.redirectTo({
                  url: '/pages/member/jfshop_detail/jfshop_detail?id=' + that.data.shop_id,
                })
              } else {
                wx.redirectTo({
  ...
  view: function (id) {
    var that = this;
    wx.request({
      url: this.data.globalData.domain + '/wxxcx/member/address_edit_json',
      data: {
        id: id
      },
      success(res) {
        that.setData({ 'view': res.data, 'province':res.data.res.address })
        console.log(res.data);
      }
    })
  },
